...
class PartialParse(object):
    ...
    def parse_step(self, transition_id, deprel=None):
        ...
        """
        # *** BEGIN YOUR CODE ***
        if (self.left_arc_id == transition_id):
            if (len(self.stack) - 2 < 0):
                raise ValueError('Stack too small to perform left-arc')
            dependent = self.stack.pop(len(self.stack) - 2)
            if (deprel == None):
                raise ValueError('No dependecy label for operation')		
            self.arcs.append((self.stack[-1], dependent, deprel))          
        elif (self.right_arc_id == transition_id):
            if (len(self.stack) - 1 < 0):
                raise ValueError('Stack too small to perform right-arc')		
            dependent = self.stack.pop()
            if (deprel == None):
                raise ValueError('No dependecy label for operation')		
            self.arcs.append((self.stack[-1], dependent, deprel))	
        elif (self.shift_id == transition_id):
            if(self.next == len(self.sentence)):
                raise ValueError('Cannot shift anymore')			
    ...
    def get_n_leftmost_deps(self, sentence_idx, n=None):
        ...
        # *** BEGIN YOUR CODE ***
        deps = []
        #for x in range(0, sentence_idx + 1):
        for x in range(len(self.arcs)):
            if(self.arcs[x][0] == sentence_idx):
                deps.append(self.arcs[x][1])
        deps.sort()
        if n != None and len(deps) > n:
            deps = deps[:n]
        # *** END YOUR CODE ***
        return deps
...
def minibatch_parse(sentences, model, batch_size):
    ...
    for x in range(len(sentences)):
        y = PartialParse(sentences[x])
        partial_list.append(y)
    unfinished_parses = copy(partial_list)

    while(len(unfinished_parses) > 0):
        to_parse = unfinished_parses[0:batch_size]
        for x in range(len(to_parse)):
            while (not to_parse[x].complete):
                td_pair = model.predict([to_parse[x]])
                to_parse[x].parse(td_pair)
            arcs.append(to_parse[x].arcs)
    
        unfinished_parses = unfinished_parses[batch_size:]
        #td_pairs = model.predict(to_parse)
        #for x in range(len(to_parse)):
         #   arcs.append(to_parse[x].parse(td_pairs[x]))
    # *** END YOUR CODE ***
    return arcs
